...
class PortfolioController extends Controller
{
     ...
     */
    protected $apiTimeout = 120; // 2 menit untuk blockchain API calls
    ...
    public function index()
    {
        ...
        return view('backend.portfolio.index', [
            ...
            'apiStatus'                  => $apiStatus,
            'apiUrl'                     => $this->apiUrl, // ⚡ FIXED: Kirim apiUrl ke view
        ]);
    }
    ...
    public function loadAnalyticsData()
    {
        ...
        try {
            ...
            $analytics = Cache::remember("onchain_analytics_v2_{$walletAddress}", 30, function () use ($walletAddress) {
                try {
            ...
            $recentTransactions = Cache::remember("onchain_transactions_v2_{$walletAddress}", 15, function () use ($walletAddress) {
                try {
    ...
    public function transactionManagement()
    {
        ...
        return view('backend.portfolio.transaction_management', [
            ...
            'apiUrl'                  => $this->apiUrl, // ⚡ FIXED: Kirim apiUrl ke view jika diperlukan
        ]);
    }
    ...
    public function refreshOnchainData()
    {
        ...
        try {
            ...
            Cache::put("onchain_portfolio_v2_{$walletAddress}", $onchainPortfolio, 30); // 30 minutes
            Cache::put("onchain_analytics_v2_{$walletAddress}", $analytics, 30);        // 30 minutes

            return response()->json([
                'success'    => true,
                'portfolio'  => $onchainPortfolio,
                'analytics'  => $analytics,
                'message'    => 'Data onchain berhasil diperbarui dan di-cache',
            ...
            if (str_contains($e->getMessage(), 'timeout') || str_contains($e->getMessage(), 'Operation timed out')) {
                $errorMessage = 'Request timeout - API blockchain sedang lambat. Coba lagi dalam beberapa saat.';
